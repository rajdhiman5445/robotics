...
$$p(x\vert{}\omega_1) = \mathcal{N}(0, 1), \quad p(x\vert{}\omega_2) = \mathcal{N}(2, 1)$$
...
$$P(\omega_1\vert{}x) > P(\omega_2\vert{}x)$$


or equivalently, by applying Bayes' Theorem and dropping the common evidence term $p(x)$:


$$p(x\vert{}\omega_1)P(\omega_1) > p(x\vert{}\omega_2)P(\omega_2)$$

Since we are dealing with Gaussian distributions, it is easier to work with log-discriminant functions, $g_i(x) = \ln(p(x\vert{}\omega_i)) + \ln(P(\omega_i))$.

The Gaussian density function is: $p(x\vert{}\omega_i) = \frac{1}{\sqrt{2\pi}\sigma_i} \exp\left(-\frac{(x-\mu_i)^2}{2\sigma_i^2}\right)$
...
$$P(\omega_1 \vert{} x) = 0.18, \quad P(\omega_2 \vert{} x) = 0.82$$
...
The conditional risk $R(\alpha_i \vert{} x)$ of taking action $\alpha_i$ (deciding class $i$) is the expected loss:


$$R(\alpha_i \vert{} x) = \sum_{j=1}^{c} \lambda_{ij} P(\omega_j \vert{} x)$$

**Risk of deciding $\omega_1$ (predicting disease):**


$$R(\alpha_1 \vert{} x) = \lambda_{11}P(\omega_1\vert{}x) + \lambda_{12}P(\omega_2\vert{}x)$$

$$R(\alpha_1 \vert{} x) = (0)(0.18) + (1)(0.82)$$

$$R(\alpha_1 \vert{} x) = 0.82$$

**Risk of deciding $\omega_2$ (predicting healthy):**


$$R(\alpha_2 \vert{} x) = \lambda_{21}P(\omega_1\vert{}x) + \lambda_{22}P(\omega_2\vert{}x)$$

$$R(\alpha_2 \vert{} x) = (20)(0.18) + (0)(0.82)$$

$$R(\alpha_2 \vert{} x) = 3.6$$
...
Since $R(\alpha_1 \vert{} x) = 0.82$ and $R(\alpha_2 \vert{} x) = 3.6$, the minimum risk is associated with action $\alpha_1$.
...
Since $P(\omega_2 \vert{} x) = 0.82 > P(\omega_1 \vert{} x) = 0.18$, a minimum-error classifier would choose **$\omega_2$ (healthy)**.
...
We want to find the threshold $P^*$ such that we predict $\omega_1$ (disease) if $P(\omega_1\vert{}x) > P^*$.
We predict $\omega_1$ if $R(\alpha_1 \vert{} x) < R(\alpha_2 \vert{} x)$.

Using the formulas from part (a):


$$\lambda_{11}P(\omega_1\vert{}x) + \lambda_{12}P(\omega_2\vert{}x) < \lambda_{21}P(\omega_1\vert{}x) + \lambda_{22}P(\omega_2\vert{}x)$$

We know that for a two-class problem, $P(\omega_2\vert{}x) = 1 - P(\omega_1\vert{}x)$. Substitute this in:


$$\lambda_{11}P(\omega_1\vert{}x) + \lambda_{12}(1 - P(\omega_1\vert{}x)) < \lambda_{21}P(\omega_1\vert{}x) + \lambda_{22}(1 - P(\omega_1\vert{}x))$$

Rearrange to solve for $P(\omega_1\vert{}x)$:


$$\lambda_{12} - \lambda_{12}P(\omega_1\vert{}x) + \lambda_{11}P(\omega_1\vert{}x) < \lambda_{22} - \lambda_{22}P(\omega_1\vert{}x) + \lambda_{21}P(\omega_1\vert{}x)$$

$$\lambda_{12} - \lambda_{22} < P(\omega_1\vert{}x)(\lambda_{21} - \lambda_{22} + \lambda_{12} - \lambda_{11})$$

$$P(\omega_1\vert{}x) > \frac{\lambda_{12} - \lambda_{22}}{(\lambda_{21} - \lambda_{11}) + (\lambda_{12} - \lambda_{22})}$$

Assuming standard conditions where correct classifications have zero loss ($\lambda_{11} = 0, \lambda_{22} = 0$):


$$P(\omega_1\vert{}x) > \frac{\lambda_{12}}{\lambda_{21} + \lambda_{12}}$$
...
$$p(x \vert{} \omega_i) = \mathcal{N}(\mu_i, \Sigma_i), \quad i=1,2$$
...
Since $p(x)$ is constant for all classes, we want to maximize $p(x\vert{}\omega_i)P(\omega_i)$.
Applying the natural logarithm gives us the general log-discriminant function:


$$g_i(x) = \ln(p(x\vert{}\omega_i)) + \ln(P(\omega_i))$$

Substitute the multivariate Gaussian density function for $p(x\vert{}\omega_i)$:


$$p(x\vert{}\omega_i) = \frac{1}{(2\pi)^{d/2}\vert{}\Sigma_i\vert{}^{1/2}} \exp\left(-\frac{1}{2}(x-\mu_i)^T\Sigma_i^{-1}(x-\mu_i)\right)$$

Taking the natural log of this density:


$$\ln(p(x\vert{}\omega_i)) = -\frac{1}{2}(x-\mu_i)^T\Sigma_i^{-1}(x-\mu_i) - \frac{d}{2}\ln(2\pi) - \frac{1}{2}\ln\vert{}\Sigma_i\vert{}$$

Substituting this back into the general discriminant function yields:


$$g_i(x) = -\frac{1}{2}(x-\mu_i)^T\Sigma_i^{-1}(x-\mu_i) - \frac{d}{2}\ln(2\pi) - \frac{1}{2}\ln\vert{}\Sigma_i\vert{} + \ln(P(\omega_i))$$
...
The term $-\frac{1}{2}\ln\vert{}\Sigma\vert{}$ is now the same for both classes and can be ignored.
...
$$(x-\mu_i)^T\Sigma^{-1}(x-\mu_i) = x^T\Sigma^{-1}x - x^T\Sigma^{-1}\mu_i - \mu_i^T\Sigma^{-1}x + \mu_i^T\Sigma^{-1}\mu_i$$


Since $\Sigma^{-1}$ is symmetric, $x^T\Sigma^{-1}\mu_i = \mu_i^T\Sigma^{-1}x$, so:
...
The remaining terms can be grouped into the form of a linear equation $w_i^Tx + w_{i0}$:
Let $w_i = \Sigma^{-1}\mu_i$
Let $w_{i0} = -\frac{1}{2}\mu_i^T\Sigma^{-1}\mu_i + \ln(P(\omega_i))$

Then $g_i(x) = w_i^Tx + w_{i0}$. Because the discriminant function is linear with respect to $x$, the decision boundary where $g_1(x) = g_2(x)$ will be a linear hyperplane.
...
$$g_i(x) = -\frac{1}{2}(x-\mu_i)^T\Sigma_i^{-1}(x-\mu_i) - \frac{1}{2}\ln\vert{}\Sigma_i\vert{} + \ln(P(\omega_i))$$
...
$$g_i(x) = -\frac{1}{2}x^T\Sigma_i^{-1}x + \mu_i^T\Sigma_i^{-1}x - \frac{1}{2}\mu_i^T\Sigma_i^{-1}\mu_i - \frac{1}{2}\ln\vert{}\Sigma_i\vert{} + \ln(P(\omega_i))$$

We can group this into a quadratic form $x^TW_ix + w_i^Tx + w_{i0}$:
Let $W_i = -\frac{1}{2}\Sigma_i^{-1}$
Let $w_i = \Sigma_i^{-1}\mu_i$
Let $w_{i0} = -\frac{1}{2}\mu_i^T\Sigma_i^{-1}\mu_i - \frac{1}{2}\ln\vert{}\Sigma_i\vert{} + \ln(P(\omega_i))$

Then $g_i(x) = x^TW_ix + w_i^Tx + w_{i0}$.
When we set $g_1(x) = g_2(x)$ to find the boundary, the quadratic term $x^TW_1x$ will not cancel out with $x^TW_2x$ because $W_1 \neq W_2$ (since $\Sigma_1 \neq \Sigma_2$). The resulting equation remains a second-order polynomial in $x$, defining a quadratic decision boundary (such as an ellipse, parabola, or hyperbola).
